...
public class query {
    ...
    public ResultSet TopRecommendations(int customerid) {
        ...
        try {
            ...
            "ON customer_ratings.titleid=titles.titleid WHERE customer_ratings.titleid IN (SELECT DISTINCT titleid FROM customer_ratings " + 
            "WHERE titleid NOT IN (SELECT titleid FROM customer_ratings WHERE customerid = " + customerid + ") AND customerid IN " + 
            "(SELECT customerid FROM customer_ratings WHERE customerid!=" + customerid + " AND rating>=4 AND titleid IN " + 
            "(SELECT titleid FROM customer_ratings WHERE customerid=" + customerid + " AND rating>=4 ORDER BY rating DESC LIMIT 10) LIMIT 10) " + 
            "AND rating >= 4 LIMIT 20) ORDER BY averagerating DESC;"
